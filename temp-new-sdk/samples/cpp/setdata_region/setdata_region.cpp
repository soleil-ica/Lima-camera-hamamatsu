...
void get_maxsize( HDCAM hdcam, int32& width, int32& height )
{
	...
	attr.iProp	= DCAM_IDPROP_IMAGE_WIDTH;

	err = dcamprop_getattr( hdcam, &attr );
	ASSERT( !failed(err) );
	
	width = (int32)attr.valuemax;

	memset( &attr, 0, sizeof(attr) );
	attr.cbSize	= sizeof(attr);
	attr.iProp	= DCAM_IDPROP_IMAGE_HEIGHT;

	err = dcamprop_getattr( hdcam, &attr );
	ASSERT( !failed(err) );
	
	height = (int32)attr.valuemax;
}
	...
	if( regiontype & DCAMDATA_REGIONTYPE__RECT16ARRAY )
	{
		...
		if( attribute & DCAMDATA_REGIONTYPE__HASVIEW )
		{
			...
			for( i=1; i<=nView; i++ )
			{
				...
				set_rectparam( pRect, nRect, 0, 0, 99, 99 );	// 100 x 100
